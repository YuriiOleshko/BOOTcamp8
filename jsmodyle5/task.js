...
const getUsersByEyeColor = (arr, color) => {
  let res = arr.filter(el=>el.eyeColor==color)
  return res
};
...
const getUsersByGender = (arr, gender) => {
  let res = arr.filter(el=>el.gender==gender).map(el=>el.name)
  return res;
};
...
const getInactiveUsers = arr => {
  let res = arr.filter(el=>el.isActive===false)
  return res 
};
...
const getUserByEmail = (arr, email) => {
  let res =arr.filter(el=>el.email==email) ;
  return res ;
};
...
const getUsersWithAge = (arr, min, max) => {
  let res = arr.filter(el=>el.age>=min && el.age<=max );
  return res
};
...
const getTotalBalance = arr => {
  let res =arr.reduce((acc,el)=>el.balance+acc,0)
  return res;
};
...
const getUsersByFriend = (arr, name) => {
  let res = arr.filter(el=>el.friends.includes(name)).map(el=>el.name)
  return res 
};
