...
// /**
//  * Класс, объекты которого описывают параметры гамбу ргера.
//  */
class Hamburger {
    // /**
    //  * @constructor
    ...
    addTopping(topping) {
      console.log(this._toppings.length);
        if(!this._toppings.includes(topping)){
            return this._toppings.push(topping)
            console.log(this._toppings[0]);
        }
    }
    ...
    //  */
    calculatePrice() {
      // for (let ele of this._toppings){
        let yrt=Hamburger.STUFFINGS[this._stuffing].price
        let opp =Hamburger.SIZES[this._size].price
       ;
        // let res = this._toppings.reduce((acc,el)=>acc+ 
        // Hamburger.TOPPINGS[el].price,0)
    if (this._toppings.length>0){
      let res = this._toppings.reduce((acc,el)=>acc+ Hamburger.TOPPINGS[el].price,0)
    ...
    calculateCalories() {
      ...
      let yrt=Hamburger.STUFFINGS[this._stuffing].calories
      let opp =Hamburger.SIZES[this._size].calories
      if (this._toppings.length>0){
      
        let res = this._toppings.reduce((acc,el)=>acc+ Hamburger.TOPPINGS[el].calories,0)
